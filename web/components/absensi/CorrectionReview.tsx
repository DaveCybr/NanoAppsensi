    ...
    <div className="bg-card rounded-2xl border-2 border-primary/10 shadow-sm overflow-hidden text-left relative">
      ...
      <div className="divide-y divide-primary/5">
        {corrections?.map((item: any) => (
          <div key={item.id} className="p-6 hover:bg-muted/30 transition-colors flex flex-col gap-6">
            <div className="flex-1 space-y-3">
              <div className="flex items-center gap-3">
                <span className="text-sm font-bold">{item.employee?.full_name}</span>
                <span className="text-xs text-muted-foreground">• {formatDate(item.attendance?.attendance_date, 'short')}</span>
              </div>
            ...
            <div className="flex items-center gap-2">
              <button 
                ...
                className="flex-1 px-4 py-2 bg-green-600 text-white rounded-lg text-xs font-bold shadow-lg shadow-green-600/20 hover:bg-green-700 transition-all flex items-center justify-center gap-2"
              >
                <Check className="w-3.5 h-3.5" /> Setujui
              </button>
              <button 
                onClick={() => handleAction(item.id, 'reject')}
                className="flex-1 px-4 py-2 bg-white border border-red-200 text-red-600 rounded-lg text-xs font-bold hover:bg-red-50 transition-all flex items-center justify-center gap-2"
              >
