 ...
 */
// PATCH: Ganti fungsi createEmployee di web/src/lib/employeeService.ts
//
// Hapus fungsi createEmployee yang lama (baris mulai "export async function createEmployee")
...
export async function createEmployee(
  ...
  // ── 1. Buat auth user via Admin API (tidak logout admin) ──────────────────
  if (form.email && form.password) {
    const { userId: newUserId, error: authError } = await createAuthUser(
      form.email,
      form.password,
      { full_name: form.full_name, tenant_id: tenantId },
    );
    ...
    // ── 2. Insert row ke public.users ────────────────────────────────────────
    // Supabase Auth trigger biasanya handle ini, tapi kalau tidak ada trigger
    // kita insert manual. Gunakan upsert agar aman jika trigger sudah jalan.
    if (userId) {
      ...
      if (userRowError) {
        console.warn(
          "[createEmployee] Failed to upsert users row:",
          userRowError.message,
        );
        // Tidak block — employee record tetap dibuat
      }
    }
  }
