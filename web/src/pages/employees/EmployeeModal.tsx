...
import { X, Loader2, AlertCircle, User, Briefcase, CreditCard } from 'lucide-react'
...
const EMPTY_FORM: EmployeeFormData = {
  full_name: '', employee_code: '', birth_date: '', gender: '',
  marital_status: '', national_id: '', phone: '', address: '',
  department_id: '', position_id: '', work_location_id: '',
  employment_status: 'active', hire_date: '', manager_id: '',
  email: '', password: '',
}
...
export default function EmployeeModal({ employee, onClose, onSave, isSaving, saveError }: Props) {
  const isEdit = !!employee
  const [step, setStep]   = useState<Step>('personal')
  const [form, setForm]   = useState<EmployeeFormData>(EMPTY_FORM)
  const { departments, positions, workLocations, managers } = useEmployeeFormData()
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                ...
                <input value={form.full_name} onChange={e => set('full_name', e.target.value)}
                  className="input text-sm" placeholder="Nama lengkap karyawan" required />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1">Kode Karyawan</label>
                <input value={form.employee_code} onChange={e => set('employee_code', e.target.value)}
                  className="input text-sm" placeholder="e.g. EMP001" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1">No. HP</label>
                <input value={form.phone} onChange={e => set('phone', e.target.value)}
                  className="input text-sm" placeholder="08xxxxxxxxxx" type="tel" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1">Tanggal Lahir</label>
                <input value={form.birth_date} onChange={e => set('birth_date', e.target.value)}
                  className="input text-sm" type="date" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1">Jenis Kelamin</label>
                <select value={form.gender} onChange={e => set('gender', e.target.value)} className="select text-sm">
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1">Status Perkawinan</label>
                <select value={form.marital_status} onChange={e => set('marital_status', e.target.value)} className="select text-sm">
                  <option value="">Pilih</option>
              ...
              <div>
                ...
                <input value={form.national_id} onChange={e => set('national_id', e.target.value)}
                  className="input text-sm" placeholder="16 digit NIK" maxLength={16} />
              </div>
              <div className="col-span-2">
                <label className="block text-xs font-semibold text-gray-600 mb-1">Alamat</label>
                <textarea value={form.address} onChange={e => set('address', e.target.value)}
                  className="input text-sm resize-none" rows={2} placeholder="Alamat lengkap" />
              </div>
            </div>
          )}
          ...
          {step === 'employment' && (
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select value={form.department_id} onChange={e => set('department_id', e.target.value)} className="select text-sm">
              ...
              <div>
                ...
                <select value={form.position_id} onChange={e => set('position_id', e.target.value)} className="select text-sm">
              ...
              <div>
                ...
                <select value={form.work_location_id} onChange={e => set('work_location_id', e.target.value)} className="select text-sm">
              ...
              <div>
                ...
                <select value={form.employment_status} onChange={e => set('employment_status', e.target.value)} className="select text-sm">
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1">Tanggal Bergabung</label>
                <input value={form.hire_date} onChange={e => set('hire_date', e.target.value)}
                  className="input text-sm" type="date" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1">Atasan Langsung</label>
                <select value={form.manager_id} onChange={e => set('manager_id', e.target.value)} className="select text-sm">
          ...
          {step === 'account' && (
            <div className="space-y-4">
              <div className="p-3 bg-blue-50 border border-blue-100 rounded-lg text-xs text-blue-700 leading-relaxed">
                Akun ini digunakan karyawan untuk login di aplikasi mobile. Email konfirmasi akan dikirim otomatis oleh Supabase.
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1">Email {!isEdit && '*'}</label>
                <input value={form.email} onChange={e => set('email', e.target.value)}
                  className="input text-sm" type="email" placeholder="karyawan@nano.co.id"
                  required={!isEdit} disabled={isEdit} />
                {isEdit && <p className="text-[11px] text-gray-400 mt-1">Email tidak dapat diubah setelah akun dibuat.</p>}
              </div>
              <div>
                ...
                <input value={form.password} onChange={e => set('password', e.target.value)}
                  className="input text-sm" type="password" placeholder="Min. 8 karakter"
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-gray-100 shrink-0">
          ...
          {isLastStep ? (
            <button onClick={handleSubmit} disabled={isSaving} className="btn-primary text-xs">
              {isSaving ? <><Loader2 size={13} className="animate-spin" /> Menyimpan...</> : (isEdit ? 'Simpan Perubahan' : 'Buat Karyawan')}
            </button>
          ) : (
