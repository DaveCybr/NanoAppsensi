...
export default function EmployeePage() {
  ...
  const { employees, totalCount, isLoading, error, filters, setFilter, refetch } =
    useEmployeeList()

  const { create, update, remove, toggleActive, isSaving, isDeleting, saveError, setSaveError } =
  ...
  const handleStatusFilter = (val: typeof statusFilter) => {
    setStatusFilter(val)
    setFilter({ isActive: val === 'all' ? undefined : val === 'active', page: 1 })
  }
  const openCreate = () => { setEditTarget(null); setSaveError(null); setShowModal(true) }
  const openEdit = (emp: EmployeeWithRelations) => { setEditTarget(emp); setSaveError(null); setShowModal(true) }

  const handleSave = async (form: EmployeeFormData): Promise<{ error: string | null }> => {
    const result = editTarget ? await update(editTarget.id, form) : await create(form)
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="card overflow-hidden">
        ...
        <div className="px-4 py-3 border-b border-gray-100 flex items-center gap-3 flex-wrap">
          ...
          <button className="btn-secondary text-xs"><Download size={14} /> Download</button>
        ...
        <div className="px-4 py-3 border-b border-gray-100 flex flex-wrap gap-2 items-center">
          ...
          <button onClick={handleSearch} className="btn-secondary text-xs">Cari</button>
            ...
            <tbody>
              ...
              {!isLoading && employees.length === 0 && (
                <tr><td colSpan={8} className="py-12 text-center text-sm text-gray-400">
                  {filters.search ? `Tidak ada hasil untuk "${filters.search}"` : 'Belum ada karyawan'}
                </td></tr>
              )}
              {!isLoading && employees.map(emp => (
                <tr key={emp.id} className="table-tr-hover">
                  <td className="table-td text-xs font-mono text-gray-400">{emp.employee_code ?? '—'}</td>
                    ...
                    <div className="flex items-center gap-2.5">
                      <div className="w-7 h-7 rounded-full bg-gradient-to-br from-blue-400 to-indigo-500 flex items-center justify-center shrink-0 overflow-hidden">
                        {emp.photo_url
                          ...
                          : <span className="text-white text-[10px] font-bold">{emp.full_name.charAt(0)}</span>
                        }
                      </div>
                      <div>
                        <div className="text-xs font-medium text-gray-800">{emp.full_name}</div>
                        <div className="text-[11px] text-gray-400">{emp.users?.email ?? '—'}</div>
                      </div>
                    </div>
                  </td>
                  <td className="table-td text-xs text-gray-600">{emp.departments?.name ?? <span className="text-gray-300">—</span>}</td>
                  <td className="table-td text-xs text-gray-600">{emp.positions?.name ?? <span className="text-gray-300">—</span>}</td>
                  <td className="table-td text-xs text-gray-600">{emp.work_locations?.name ?? <span className="text-gray-300">—</span>}</td>
                  <td className="table-td">
                    <span className={clsx('badge text-xs',
                      emp.employment_status === 'active'    ? 'badge-success' :
                      emp.employment_status === 'probation' ? 'badge-info' :
                      emp.employment_status === 'contract'  ? 'badge-warning' : 'badge-gray'
                    )}>{emp.employment_status ?? '—'}</span>
                  </td>
                  <td className="table-td">
                    <button onClick={() => emp.users?.id && toggleActive(emp.users.id, !emp.users.is_active)}
                      ...
                      <span className={clsx('inline-block h-3.5 w-3.5 transform rounded-full bg-white shadow transition-transform',
                        emp.users?.is_active ? 'translate-x-4' : 'translate-x-0.5')} />
                    </button>
                  </td>
                  <td className="table-td">
        ...
        <div className="px-4 py-3 border-t border-gray-100 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <select value={filters.perPage} onChange={e => setFilter({ perPage: Number(e.target.value), page: 1 })}
              className="select text-xs w-16">
              {[10,25,50].map(n => <option key={n}>{n}</option>)}
            </select>
          </div>
          <div className="flex items-center gap-1 text-xs text-gray-500">
            <span>{((filters.page??1)-1)*(filters.perPage??10)+1}–{Math.min((filters.page??1)*(filters.perPage??10), totalCount)} dari {totalCount}</span>
            <button onClick={() => setFilter({ page: 1 })} disabled={(filters.page??1)===1} className="btn-icon">«</button>
            <button onClick={() => setFilter({ page: (filters.page??1)-1 })} disabled={(filters.page??1)===1} className="btn-icon">‹</button>
            <button onClick={() => setFilter({ page: (filters.page??1)+1 })} disabled={(filters.page??1)===totalPages} className="btn-icon">›</button>
            <button onClick={() => setFilter({ page: totalPages })} disabled={(filters.page??1)===totalPages} className="btn-icon">»</button>
          </div>
        </div>
      </div>

      {showModal && (
        <EmployeeModal employee={editTarget} onClose={() => setShowModal(false)}
          onSave={handleSave} isSaving={isSaving} saveError={saveError} />
      )}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-sm">
            ...
            <p className="text-sm text-gray-500 mb-5">
              <strong className="text-gray-700">{deleteTarget.full_name}</strong> akan dinonaktifkan (soft delete). Data absensi tetap tersimpan.
            </p>
            <div className="flex gap-2 justify-end">
              <button onClick={() => setDeleteTarget(null)} className="btn-secondary text-xs" disabled={isDeleting}>Batal</button>
              <button onClick={handleDelete} className="btn-danger text-xs" disabled={isDeleting}>
                {isDeleting ? <><Loader2 size={13} className="animate-spin" /> Menghapus...</> : 'Hapus'}
              </button>
            </div>
          </div>
        </div>
