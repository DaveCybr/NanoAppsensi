...
export default function ZonesPage() {
  ...
  useEffect(() => {
    ...
    // FIX 5: guard against React StrictMode double-invoke on overview map
    ...
    import("leaflet").then((L) => {
      ...
      const street = L.tileLayer(STREET_TILE_URL, {
    ...
    return () => {
      if (mapInstanceRef.current) {
        ...
        streetTileRef.current = null;
        satelitTileRef.current = null;
      }
    };
  }, []);
  ...
  useEffect(() => {
    ...
    if (mapLayer === "peta") {
      if (map.hasLayer(satelit)) satelit.remove();
      if (!map.hasLayer(street)) street.addTo(map);
    } else {
      if (map.hasLayer(street)) street.remove();
      if (!map.hasLayer(satelit)) satelit.addTo(map);
    }
  }, [mapLayer]);
  ...
  useEffect(() => {
    ...
    // FIX 5: Use a cleanup flag to handle the case where the modal is closed
    // before the async import resolves, preventing a stale map from being
    // created and left dangling. Also guard against _leaflet_id to survive
    // React StrictMode's double-invoke.
    let cancelled = false;

    const timer = setTimeout(() => {
      if (cancelled || !modalMapRef.current || modalMapInstanceRef.current)
        return;
      if ((modalMapRef.current as any)._leaflet_id) return;
      ...
      import("leaflet").then((L) => {
        ...
        L.tileLayer(STREET_TILE_URL, { attribution: "© OpenStreetMap" }).addTo(
          map,
        );
    ...
    return () => {
      cancelled = true;
      clearTimeout(timer);
      // Always destroy the modal map on cleanup so next open gets a fresh instance
  ...
  // ── Update marker + circle when coords/radius change ──────────────────────
  useEffect(() => {
    ...
    import("leaflet").then((L) => {
      if (!modalMapInstanceRef.current) return; // guard: modal may have closed
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="card overflow-hidden" style={{ isolation: "isolate" }}>
        ...
        <div
          ref={mapRef}
          style={{ height: 300, width: "100%", marginTop: -40 }}
        />
      </div>
      ...
      <div className="card overflow-hidden">
        {error && (
          <div className="m-4 p-3 bg-red-50 border border-red-200 rounded-lg text-xs text-red-600">
            {error}
          </div>
        )}
        <div className="p-4 border-b border-gray-100 flex items-center gap-3">
          <button onClick={openAdd} className="btn-primary text-xs">
            <Plus size={14} /> Add Zones
          </button>
            ...
            {!isLoading &&
              filtered.map((z) => (
                <tr key={z.id} className="table-tr-hover">
                  ...
                  <td className="table-td text-xs font-semibold text-gray-800">
                    {z.radius_meters}
                  </td>
                  <td className="table-td">
        ...
        <div className="px-4 py-3 border-t border-gray-100 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <select className="select text-xs w-16">
              <option>10</option>
            </select>
          </div>
                ...
                {[
                  {
                    label: "Latitude",
                    key: "latitude",
                    placeholder: "e.g. -8.200565",
                  },
                  {
                    label: "Longitude",
                    key: "longitude",
                    placeholder: "e.g. 113.6792966",
                  },
                ].map((f) => (
                  <div key={f.key}>
                    <label className="text-xs font-medium text-gray-600 mb-1 block">
        ...
        <div className="fixed inset-0 bg-black/40 z-[1000] flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-dropdown w-full max-w-sm p-6">
            ...
            <p className="text-sm text-gray-500 mb-5">
              Data zona ini akan dihapus secara permanen.
            </p>
